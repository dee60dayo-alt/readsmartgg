...
const CustomQuestions = () => {
  ...
  const handleSelect = (i: number) => {
    ...
    setSelected(i);
    setAnswered(true);
    if (i === questions[playIdx].correct) setScore((s) => s + 1);
  };

  const handleNext = () => {
    if (playIdx + 1 >= questions.length) {
      setFinished(true);
    } else {
      setPlayIdx((i) => i + 1);
      setSelected(null);
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        <motion.div className="bg-card rounded-2xl p-8 shadow-lg border border-border max-w-md w-full text-center" initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}>
          <Trophy className="w-16 h-16 text-gold mx-auto mb-4" strokeLinejoin="miter" strokeLinecap="square" />
      ...
      <div className="min-h-screen bg-background">
        <header className="sticky top-0 z-50 bg-card/80 backdrop-blur-lg border-b border-border shadow-sm">
          <div className="max-w-2xl mx-auto px-4 py-3 flex items-center justify-between">
            <button onClick={() => setMode("list")} className="flex items-center gap-2 text-foreground hover:text-primary transition-colors">
                ...
                {current.options.map((opt, i) => {
                  ...
                  return (
                    <motion.button key={i} className={`w-full text-left p-4 rounded-xl border-2 ${borderClass} ${bgClass} transition-colors flex items-center gap-3`} onClick={() => handleSelect(i)}>
                      ...
                      {answered && i === current.correct && <CheckCircle2 className="w-5 h-5 text-primary shrink-0" strokeLinejoin="miter" strokeLinecap="square" />}
                      {answered && i === selected && i !== current.correct && <XCircle className="w-5 h-5 text-destructive shrink-0" strokeLinejoin="miter" strokeLinecap="square" />}
                    </motion.button>
                  );
                })}
              </div>
        ...
        <main className="max-w-2xl mx-auto px-4 py-6 space-y-4">
          <div>
            ...
            <textarea
              ...
              className="w-full bg-card border border-border rounded-xl p-3 text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary resize-none"
              rows={3}
            ...
            <div key={i} className="flex items-center gap-3">
              <button
                ...
                className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold shrink-0 transition-colors ${
                  correctIdx === i ? "bg-primary text-primary-foreground" : "bg-muted text-foreground"
                }`}
              >
              ...
              <input
                ...
                className="flex-1 bg-card border border-border rounded-xl p-3 text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary"
                placeholder={`Option ${String.fromCharCode(65 + i)}`}
                maxLength={200}
              />
            </div>
          ))}
          <p className="text-xs text-muted-foreground">Tap a letter to mark the correct answer (highlighted in blue)</p>
          <button
            onClick={handleSave}
            disabled={!newQ.trim() || opts.some((o) => !o.trim())}
            className="w-full bg-primary text-primary-foreground font-display font-bold text-lg py-4 rounded-xl hover:opacity-90 transition-opacity disabled:opacity-40"
          >
            Save Question ✓
          </button>
        </main>
      ...
      <main className="max-w-2xl mx-auto px-4 py-6 space-y-4">
        <div className="flex gap-3">
          <button onClick={() => setMode("create")} className="flex-1 bg-primary text-primary-foreground font-bold py-3 rounded-xl hover:opacity-90 transition-opacity flex items-center justify-center gap-2">
            <Plus className="w-5 h-5" strokeLinejoin="miter" strokeLinecap="square" /> New Question
          </button>
          {questions.length >= 2 && (
            <button onClick={startPlay} className="flex-1 bg-gold text-gold-foreground font-bold py-3 rounded-xl hover:opacity-90 transition-opacity flex items-center justify-center gap-2">
              <Play className="w-5 h-5" strokeLinejoin="miter" strokeLinecap="square" /> Play ({questions.length})
            </button>
          )}
        </div>
            ...
            <div className="flex items-start justify-between gap-2">
              <div className="flex-1">
                ...
                <div className="grid grid-cols-2 gap-1">
                  {q.options.map((opt, j) => (
                    <span key={j} className={`text-xs px-2 py-1 rounded ${j === q.correct ? "bg-primary/10 text-primary font-bold" : "text-muted-foreground"}`}>
                      {String.fromCharCode(65 + j)}. {opt}
                    </span>
                  ))}
                </div>
              </div>
              <button onClick={() => handleDelete(q.id)} className="text-destructive hover:text-destructive/80 p-1">
                <Trash2 className="w-4 h-4" strokeLinejoin="miter" strokeLinecap="square" />
              </button>
            </div>
          </motion.div>
        ))}
